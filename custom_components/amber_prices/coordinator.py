...
type AmberConfigEntry = ConfigEntry[AmberUpdateCoordinator] # pylint: disable=used-before-assignment
...
def is_previous(interval: ActualInterval | CurrentInterval | ForecastInterval) -> bool:
...
class AmberUpdateCoordinator(DataUpdateCoordinator):
    ...
    def update_price_data(self) -> dict[str, dict[str, Any]]:
        """Update callback."""
        now = datetime.now()
        if self.previous_result and now.minute != self.skip_until:
            # LOGGER.debug("Skipping minute %s until %s", now.minute, self.skip_until)
        ...
        previous = [interval for interval in intervals if is_previous(interval)]
        ...
        result["previous"]["general"] = [
            interval for interval in previous if is_general(interval)
        ]
        ...
        if controlled_load:
            ...
            result["previous"]["controlled_load"] = [
                interval for interval in previous if is_controlled_load(interval)
            ]
        ...
        if feed_in:
            ...
            result["previous"]["feed_in"] = [
                interval for interval in previous if is_feed_in(interval)
            ]

        # LOGGER.debug("Fetched new Amber data: %s", intervals)
        # LOGGER.debug("Fetched new Amber data forecast=%s", result["forecasts"]["general"][0])
        LOGGER.debug("Fetched new Amber data")
        self.previous_result = result
        if not general[0].estimate:
            self.skip_until = (now.minute - (now.minute % 5) + 5) % 60
            LOGGER.debug("Skipping until minute %s", self.skip_until)
        return result
