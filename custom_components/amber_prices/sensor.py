...
class AmberPriceSensor(AmberSensor):
    ...
    def extra_state_attributes(self) -> dict[str, Any] | None:
        ...
        if interval.tariff_information is not None:
            data["tarrif_period"] = interval.tariff_information.period
            data["tarrif_season"] = interval.tariff_information.season
...
class AmberForecastSensor(AmberSensor):
    ...
    def extra_state_attributes(self) -> dict[str, Any] | None:
        ...
        for interval in intervals:
            ...
            datum["per_kwh"] = multiplier * format_cents_to_dollars(interval.per_kwh)
            ...
            if interval.advanced_price is not None:
                datum["advanced_price_low"] = multiplier * format_cents_to_dollars(interval.advanced_price.low)
                datum["advanced_price_predicted"] = multiplier * format_cents_to_dollars(interval.advanced_price.predicted)
                datum["advanced_price_high"] = multiplier * format_cents_to_dollars(interval.advanced_price.high)
...
class AmberPreviousSensor(AmberSensor):
    """Amber Previous Sensor."""

    @property
    def native_value(self) -> float | None:
        """Return the first price in $/kWh."""
        intervals = self.coordinator.data[self.entity_description.key].get(
            self.channel_type
        )
        if not intervals:
...
async def async_setup_entry(
    ...
    previous: dict[str, list[ForecastInterval]] = coordinator.data["previous"]
    ...
    for channel_type in previous:
        ...
        entities.append(AmberPreviousSensor(coordinator, description, channel_type))
